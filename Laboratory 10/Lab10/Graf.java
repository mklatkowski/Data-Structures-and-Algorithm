...
public class Graf<T> {
    ...
    class WierzcholekC<T> {
        Wierzcholek<T> wierzcholek;
        COLOR color;
        boolean isDone;

        public WierzcholekC(Wierzcholek<T> wierzcholek, COLOR color) {
            this.wierzcholek = wierzcholek;
            this.color = color;
            isDone = false;
        }

        public Wierzcholek<T> getWierzcholek() {
            return wierzcholek;
        }

        public void setWierzcholek(Wierzcholek<T> wierzcholek) {
            this.wierzcholek = wierzcholek;
        }
    ...
    private ArrayList<Krawedz<T>> drzewoRozpinajace() {
        ...
        while (zbiorRoboczy.size() > 1) {
            System.out.println(zbiorRoboczy);
    ...
    public ArrayList<Wierzcholek<T>> copyWierzcholki2() {
        ...
        for (int i = 0; i < wierzcholki.size(); i++) {
            list.add(new Wierzcholek<>(wierzcholki.get(i).getWierzcholek()));
        }
        return list;
    }
...
//            }
    ...
    public void wszystkieSciezki(T start, T end) {
        System.out.println("INNE DROGI:");
        boolean st = false;
        boolean en = false;
        for (Wierzcholek<T> w : wierzcholki) {
            if (w.getWierzcholek().equals(start)) {
                st = true;
            }
            if (w.getWierzcholek().equals(end)) {
                en = true;
            }
        }
        if (!st || !en) {
            System.out.println("Nie ma takich wierzchołków");
        } else {
            ArrayList<Wierzcholek<T>> tempList = copyWierzcholki2();
            ArrayList<Wierzcholek<T>> result = new ArrayList<>();
            tempList.remove(new Wierzcholek<>(start));
            result.add(new Wierzcholek<>(start));
            sciezki(start, end, tempList, result, 0);
        }
    }

    private int sciezki(T start, T end, ArrayList<Wierzcholek<T>> tempList, ArrayList<Wierzcholek<T>> result, int value) {
        if (end.equals(start)) {
            for (Wierzcholek<T> w : result) {
                System.out.println(w.getWierzcholek());
            }
            System.out.println();
            System.out.println("Całkowita droga: " + value);
            System.out.println();
            System.out.println();
            return 1;
        } else {
            int counter = 0;
            for (Wierzcholek<T> w : tempList) {
                for (Krawedz<T> kr : krawedzie) {
                    if ((kr.getKoniec().equals(start) && kr.getPoczatek().equals(w.getWierzcholek())) || kr.getPoczatek().equals(start) && kr.getKoniec().equals(w.getWierzcholek())) {
                        ArrayList<Wierzcholek<T>> tempList1 = new ArrayList<>();
                        ArrayList<Wierzcholek<T>> result1 = new ArrayList<>();
                        int value1 = value + kr.getValue();
                        for (Wierzcholek<T> r : result) {
                            result1.add(r);
                        }
                        result1.add(w);
                        for (Wierzcholek<T> wierzcholek : tempList) {
                            if (!wierzcholek.equals(w)) {
                                tempList1.add(new Wierzcholek<>(wierzcholek.getWierzcholek()));
                            }
                        }

                        sciezki(w.getWierzcholek(), end, tempList1, result1, value1);
//                        if(sciezki(w.getWierzcholek(), end, tempList1, result1)==1){
//                            System.out.println(w.getWierzcholek());
//                        }
                        counter++;
                    }
    ...
    public boolean dwudzielnosc() {

        ArrayList<WierzcholekC<T>> wierzcholkiC = new ArrayList<>();
        for (Wierzcholek<T> x : wierzcholki) {
            wierzcholkiC.add(new WierzcholekC<>(x, COLOR.WHITE));
        }
        WierzcholekC<T> temp = wierzcholkiC.get(0);
        temp.setColor(COLOR.BLACK);

        boolean b = false;
        while (!b) {
            ArrayList<WierzcholekC<T>> toCheck = new ArrayList<>();
            for (Krawedz<T> kr : krawedzie) {
                if (temp.getWierzcholek().getWierzcholek().equals(kr.getKoniec())) {
                    toCheck.add(findVertex(wierzcholkiC, kr.getPoczatek()));
                } else if (temp.getWierzcholek().getWierzcholek().equals(kr.getPoczatek())) {
                    toCheck.add(findVertex(wierzcholkiC, kr.getKoniec()));
                }

            }

            for (WierzcholekC<T> w : toCheck) {
                if (temp.getColor().equals(COLOR.BLACK)) {
                    if (!w.getColor().equals(COLOR.BLACK)) {
                        w.setColor(COLOR.GREY);
                    } else {
                        return false;
                    }
                } else {
                    if (!w.getColor().equals(COLOR.GREY)) {
            ...
            b = true;
            for(WierzcholekC<T> x: wierzcholkiC){
                if(!x.isDone()){
                    b = false;
                    if(!x.getColor().equals(COLOR.WHITE))
                        temp = x;
                }
            }
            if(!b && temp.isDone()){
        ...
        System.out.print("Pierwsza grupa: ");
        for(WierzcholekC<T> x: wierzcholkiC){
            if(x.getColor().equals(COLOR.GREY)){
                System.out.printf("%-10s", x.getWierzcholek().getWierzcholek());
            }
        }
        System.out.println();
        System.out.println();
        System.out.print("Druga grupa: ");
        for(WierzcholekC<T> x: wierzcholkiC){
            if(x.getColor().equals(COLOR.BLACK)){
                System.out.printf("%-10s", x.getWierzcholek().getWierzcholek());
            }
        }
        return true;
    ...
    private WierzcholekC<T> findVertex(ArrayList<WierzcholekC<T>> list, T wierzcholek) {
        for (WierzcholekC<T> x : list) {
            if (x.getWierzcholek().getWierzcholek().equals(wierzcholek)) {
                return x;
            }
        }
        return null;
